...
DateTime CURRENT_TIME = DateTime(2023, 7, 4);
...
List calculate_free_time(int deadline, int duration) {
  ...
  for (List free_time in db.free_DB) {
    int free_time_duration =
        free_time[2] - free_time[1] - 60000 * 2 * db.INTERVAL_TIME;
    int end_free_time = free_time[2] - 60000 * db.INTERVAL_TIME;
...
void arrange_task(List task, List free_time) {
  ...
  int new_begin_time = free_time[1] + 60000 * db.INTERVAL_TIME;
...
void initialize_appointment_and_schedule_DB() {
  ...
  for (int i = 0; i < time_length; i++) {
    ...
    int begin_time = CURRENT_TIME
        .add(Duration(hours: (db.RESTING_START_TIME + 24 * i)))
        .millisecondsSinceEpoch;
    int end_time = CURRENT_TIME
        .add(Duration(hours: (db.RESTING_END_TIME + 24 * (i + 1))))
        .millisecondsSinceEpoch;
