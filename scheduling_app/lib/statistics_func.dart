...
List<TaskModel> get_done_task_DB(List <TaskModel> task_DB) {
  ...
  for (int i = 0; i < task_DB.length; i++){
    task_DB[i].ifDone = true; // for debag
    task_DB[i].realDuration = task_DB[i].duration * 2; // for debag
    if (i == 5){
      task_DB[i].realDuration = task_DB[i].duration * 3;
    }
    if (db.task_DB[i].ifDone == true) {
      done_task_DB.add(db.task_DB[i]);
      // print("${task_DB[i].ifDone}, ${task_DB[i].duration}, ${task_DB[i].realDuration}");
    }
  }
  return done_task_DB;
}
...
String alert(int durationFromUI){
  ...
  if(predicted_duration > durationFromUI * 1.5){
    alert_message = "見積時間が小さいです。";
  }
  else if (predicted_duration < durationFromUI * 0.5){
    alert_message = "見積時間が大きいです。";
  }
  else {
